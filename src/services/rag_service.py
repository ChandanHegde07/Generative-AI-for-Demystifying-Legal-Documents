...
try:
    client = genai.Client(api_key=Config._GEMINI_API_KEY)  
...
class RAGService:
    def __init__(self, document_text: str):
        ...
        if os.path.exists(cache_folder_path):
            ...
            try:
                ...
                self.vector_store = self._build_and_save_vector_store(
                    document_text, cache_folder_path
                )
        else:
    ...
    def _build_and_save_vector_store(self, document_text, cache_path):
        ...
        try:
            ...
            st.warning(
                "Could not create the document's searchable index. Q&A may be limited."
            )
            return None
    ...
    def retrieve_relevant_chunks(self, query: str) -> str:
        ...
        try:
            print(
                f"DEBUG: Performing compressed retrieval (reranking) for query: '{query}'"
            )
