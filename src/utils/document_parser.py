...
class DocumentParser:
    def __init__(self):
        ...
        - Always initializes the Gemini model for fallback OCR.
        - Conditionally initializes the Google Cloud Vision client if enabled in config.
        """
    ...
    def _extract_text_pdf(self, pdf_file) -> str:
        """
        Extracts text from a PDF. If the PDF contains no selectable text (i.e., it's a scanned document),
        it automatically uses the configured OCR engine on the entire file.
        """
        reader = PyPDF2.PdfReader(pdf_file)
        text_content = [p.extract_text() for p in reader.pages]
        
        total_text_length = sum(len(t.strip()) for t in text_content if t)
        if total_text_length < 100: 
            print("DEBUG: PDF appears to be image-based. Applying full OCR...")
            pdf_file.seek(0)
            return self._ocr_scanned_pdf(pdf_file)

        return "\n".join(f"\n--- Page {i+1} ---\n{text}" for i, text in enumerate(text_content)).strip()

    def _ocr_scanned_pdf(self, pdf_file) -> str:
        """Applies OCR to an entire PDF file, ideal for scanned documents."""
    ...
    def _extract_text_image(self, image_file) -> str:
        """
        Extracts text from an image file by routing to the best available OCR engine.
        """
        if Config.USE_GOOGLE_VISION_OCR and self.vision_client:
    ...
    def _extract_text_image_google_vision(self, image_file) -> str:
        """High-accuracy OCR using Google Cloud Vision."""
        try:
    ...
    def _extract_text_image_gemini_vision(self, image_file) -> str:
        """OCR using the built-in Gemini model."""
        try:
            image_file.seek(0)
    ...
    def extract_text_from_file(self, uploaded_file: Any) -> str:
        """
        Extracts text from a single uploaded file. The PII is NOT anonymized here.
        Anonymization is handled by the AIProcessor before sending to the LLM.
        """
    ...
    def preprocess_text(self, text: str) -> str:
        """Cleans and prepares extracted text for analysis."""
        if not text:
            return ""
    ...
    def validate_extracted_text(self, text: str) -> Tuple[bool, str]:
        """Validates that the extracted text is sufficient for analysis."""
